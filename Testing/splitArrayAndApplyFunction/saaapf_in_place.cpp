...
void doMasterProc(int argc, char **argv, int rank, int proc_num) {
    ...
    int scatter_ret = MPI_Scatter(totalArray, chunkSize, MPI_INT, MPI_IN_PLACE, chunkSize, MPI_INT, MASTER_RANK, MPI_COMM_WORLD);
    MPIAssert(scatter_ret == MPI_SUCCESS);

    saaapf::processArray(totalArray, chunkSize);

    int gather_ret = MPI_Gather(MPI_IN_PLACE, chunkSize, MPI_INT, totalArray, chunkSize, MPI_INT, MASTER_RANK, MPI_COMM_WORLD);
