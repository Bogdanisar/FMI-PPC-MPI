...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        for (int r = 1; r < procNum; ++r) {
            int ret = MPI_Send(&array[r * eachSize], eachSize, MPI_INT, r, 0, MPI_COMM_WORLD);
        ...
        for (int r = 1; r < procNum; ++r) {
            int ret = MPI_Recv(&array[r * eachSize], eachSize, MPI_INT, r, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            assert(ret == MPI_SUCCESS);
        }
